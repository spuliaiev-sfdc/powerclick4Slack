...
const linkUnfurlEvent = {
  ...
  async process({ event, client, context, say }) {
    ...
    try {
      ...
      for (let lnkInd in event.links) {
        ...
        linkUnfurlEvent.kodamaRetriever.retrieve(info,
          (info, kodamaInfo) => {
            let text = linkUnfurlEvent.generateText(info);
            let furls = {};
            furls[linkData.url] = {
            ...
            const resultUnfurl = linkUnfurlEvent.app.client.chat.unfurl({
  ...
  parseInformation(linkData) {
    ...
    if (info.request.host === "portal.prod.ci.sfdc.net") {
      info.domainName = "AutomationPortal";
    ...
    if (info.request.host === "codesearch.data.sfdc.net") {
      ...
      urlParser.parseCodeSearch(info, linkData);
    }
...
module.exports = (app, config) => {
  ...
  linkUnfurlEvent.kodamaRetriever = require ('./kodamaRetriever.js')(app, config, linkUnfurlEvent);
