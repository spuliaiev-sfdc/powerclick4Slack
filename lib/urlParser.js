...
const filePackage = RegExp("^(?<module>[^\\/]+)\\/((?<submodule>.*)?java\\/src(?!java))\\/(?<package>.*).*$");
const classParser = RegExp("^((?<package>.*)?\\.)?(?<class>[A-Z][^.]+)(\.(?<method>[a-z][^.:]+))?(\:(?<line>[0-9]+))?$");
...
const urlParser = {
  ...
  parseCodeSearch(info, linkData) {
    ...
    if (prefixOffset > 0) {
      ...
      if (!info.fileFull.startsWith("//")) {
        info.fileFull = "/"+info.fileFull;
      }
  ...
  parseFullFileLocator(info) {
    ...
    if (parts && parts.groups) {
      ...
      if (parts.groups.path) {
        ...
        if (packageInfo && packageInfo.groups) {
  ...
  parseClassFQN(info, classFQN) {
    ...
    if (parts.groups) {
      ...
      if (parts.groups.method) {
        info.method = parts.groups.method;
      }
      if (parts.groups.line) {
        info.lineNumber = parts.groups.line;
      }
      info.fileType = "java";
    } else {
